...
function calcularEstadoPeso(peso_ideal){
    // Obtenemos el peso actual ingreso por el usuario
    var peso_actual = document.getElementById('txt_peso_actual').value;

    //Comparamos el peso actual y peso ideal para determinar
    //el estado de peso
    var estado_peso = "";
    var imagen ="";
    ...
    var contenedor_estado_peso = document.getElementById('cnt_resultado_texto_estado_peso');
    contenedor_estado_peso.innerHTML = "<b>Estado peso :</b>"+estado_peso;

    //Invocamos función para desplegar imágen alusiva
    mostrarImagenEstadoPeso(imagen);

}
function mostrarImagenEstadoPeso(imagen){
    //Desplegamos la imagen que corresponda
    var contenedor_imagen = document.getElementById('cnt_resultado_imagen_estado_peso');
